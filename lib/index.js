...
function default_1(styleApi, baseFile) {
    const { and, member, moduleName, name, not, unicode, isInstalledModule, hasNoMember, } = styleApi;
    ...
    const isEsmInstalledModule = (imported) => {
        const mod = imported.moduleName;
        if (mod.startsWith('.') || mod.startsWith('/')) {
            return false;
        }
        const scopedMatch = mod.match(/^(@[^\/]+\/[^\/]+)/);
        const packageName = scopedMatch ? scopedMatch[1] : mod.split('/')[0];
        let dir = path.dirname(path.resolve(baseFile));
